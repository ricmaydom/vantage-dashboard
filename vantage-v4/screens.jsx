        ...
        <tbody className={flags.microMotion ? "stagger" : ""}>
          {sortedIntel.map(i => (
            <tr key={i.id} onClick={() => openIntel(i)}>
              <td className="strong" style={{whiteSpace:"normal"}}><span className="strong">{i.title}</span><div className="muted text-sm" style={{marginTop:3, whiteSpace:"normal", fontWeight:400}}>{i.body.slice(0,120)}{i.body.length > 120 ? "…" : ""}</div></td>
              ...
              <td className="mono num" style={{textAlign:"right", whiteSpace:"nowrap", paddingRight:20}} {...(flags.relTime && i.date ? { "data-tip": VT_FMT.FULL(i.date) } : {})}>{fmtDDMMMYY(i.date)}</td>
            </tr>
          ))}
        </tbody>
      </table>
