    ...
    <div className="print-root active">
      ...
      <div className="tearsheet">
        <div className="tearsheet__head">
          ...
          <div className="tearsheet__ref">REF · {String(d.id).slice(0, 8).toUpperCase()}<br/>{VT_FMT.DATE(new Date())}</div>
        </div>
        ...
        <div className="tearsheet__foot">
          ...
          <span>R. Maydom · {VT_FMT.DATE(new Date())}</span>
        </div>
      </div>
    </div>
  );
...
const EditableField = ({ label, value, options, type, onSave, multiline }) => {
  ...
  // Convert display string ("15 Apr 26", "15-Apr-26", "2026-04-15", etc) → YYYY-MM-DD for the <input type="date">.
  const toInputDate = (s) => {
    if(!s || s === "—") return "";
    const dt = new Date(s);
    if(!isNaN(dt)) return dt.toISOString().slice(0, 10);
    // Try manual parse: "DD MMM YY" or "DD-MMM-YY"
  ...
  // Format input value (YYYY-MM-DD) → display string "DD MMM YY" for persistence.
  const fromInputDate = (s) => {
    if(!s) return "—";
    const fmt = (window.VT_FMT && window.VT_FMT.DATE) ? window.VT_FMT.DATE : null;
    return fmt ? fmt(s) : s;
  };
  const save = (v) => {
    const out = isDate ? fromInputDate(v) : v;
    setVal(out);
    setEditing(false);
    ...
    <div className={"efield" + (editing ? " efield--editing" : "") + (multiline ? " efield--multi" : "") + (label ? "" : " efield--nolabel")}>
      ...
      {!editing ? (
        ...
        <input
          ...
          defaultValue={isDate ? toInputDate(val) : val}
          onBlur={(e) => save(e.target.value)}
          onKeyDown={(e) => { if(e.key === "Escape") cancel(); else if(e.key === "Enter") save(e.target.value); }}
        />
      )}
